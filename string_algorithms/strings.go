...
func SortSentence(s string) string {
	...
	for _, word := range words {
		index := word[len(word)-1] - '0' - 1 // the index that strings should be in.
		result[index] = word[:len(word)-1]
	}
	return strings.Join(result, " ")
}
...
func CountMatches(items [][]string, ruleKey string, ruleValue string) int {
	...
	for _, item := range items {
		if item[colorIndexMap[ruleKey]] == ruleValue {
			matchCount += 1
		}
	}
	return matchCount
...
func MinMovesToSeat(seats []int, students []int) int {
	...
	for i := len(seats) - 1; i >= 0; i-- {
		minNumberCount += int(math.Abs(float64(seats[i] - students[i])))
	}
...
func DecodeMessage(key string, message string) string {
	keyAlphabetMap := make(map[uint8]uint8, 0)
	keyAlphabetMap[' '] = ' '
	var alphabetIndex uint8 = 'a'
	for _, char := range key {
		if char != ' ' {
			if _, isExist := keyAlphabetMap[uint8(char)]; !isExist {
				keyAlphabetMap[uint8(char)] = alphabetIndex
				alphabetIndex += 1
			}
		}
	}
	...
	for _, char := range message {
		result = result + fmt.Sprintf("%c", keyAlphabetMap[uint8(char)])
	}
	return result
}
